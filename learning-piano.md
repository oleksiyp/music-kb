# Learning Piano for Electronic Music

You don't need to "learn piano" in the classical sense. Your target is functional fluency in playing electronic music – to express musical ideas quickly and play with gear. That's very achievable in 1–2 years of light, focused practice.

## 🎹 Why Piano Helps in Electronic Music

- **Chord fluency** - Laying harmonic foundations of tracks
- **Left-hand bass** - Writing basslines and subs
- **Scale improvisation** - Writing melodies, pads, arps
- **Ear training** - Building musical intuition
- **Rhythm skills** - Better grooves and timing when jamming or sequencing

**Resource:** [Syntorial](https://www.syntorial.com/)

## How to Learn Piano Fast

- Do zero mistakes
- Start without time
- Slow metronome
- Don't increase speed long time till fully ready 
- Fix mistake immediately
- Stop when mistake happened
- Sleep well to memorize
- Practice every day 10-15 mins (min 5 mins)
- Choose optimal complexity, speed

**Video Guide:** [How to Learn Piano Fast](https://youtu.be/VZDmmAhQobY?si=620_a6PVS9c9ykII)

### Categories to Learn

- Reading sheet music
- Scales
- Chords
- Music theory
- Aural skills (listening)

### Common Problems to Avoid

- Tense hands
- Sitting too high/too low
- Using weird fingers
- Looking at hands (not reading music)
- Covering mistakes with sustain
- Don't use stickers

**Video Guide:** [Common Piano Problems](https://youtu.be/p00zsi71t6I?si=CoZwOZ_ROrbEIXET)

### Proper Hand Position

- Don't bend wrist
- Don't put it down
- Relaxed curved position
- Hold enough deep enough into keys

**Video Guide:** [Proper Hand Position](https://youtu.be/U4mU0uu-0Aw?si=yOH3WRF9iCTR0qNI)

---

# Learning Phases

## Phase 1: Foundations (0–6 months)

### Equipment Budget

**Month 1:** Volca Keys - 163 EUR (bought)  
**Month 2:** MIDI keyboard Arturia KeyLab Essential 61 - 231,44 EUR (includes Ableton Live Lite)

### Focus
Learn basic music theory (scales, chords), finger strength, rhythm, and play simple loops.

### Activities
- Learn C major, A minor, G major, etc.
- Practice common chord progressions (e.g. I–V–vi–IV)
- Use online tools like Chordify, Hooktheory or MusicTheoryForMusicians (practical theory)
- Start jamming with Volcas using simple chord pads from a MIDI keyboard
- **Practice:** 20–30 minutes/day, 5 days/week

### Recommended Resources

#### 🎹 Melodics (FREE tier to start)
- **Focus:** Finger control, rhythm, chord progressions
- **Usage:** Daily for 10–15 min — treat it like exercise
- **Features:** Game-like, bite-sized lessons tailored to finger drumming, chords, progressions
- **Target:** Great for beatmakers and producers
- **Cost:** Free with limited access, ~$10–15/month for full version
- **Website:** [melodics.com](https://melodics.com)

#### 📚 YouTube: Piano for Producers
- **Focus:** Learn how to build songs with chords + melody
- **Benefits:** Ideal for jamming over Volca Keys patterns
- **Content:** Emphasis on chords, scales, progressions for RnB, Lo-Fi, EDM, etc.
- **Style:** Visual, practical approach
- **Cost:** Free videos, or $100–200 full course

#### 🎥 Ableton Live Tutorials
- **Channels:** Loopop, Slynk, or YouTube official
- **Learn:** Session view, MIDI sequencing, FX usage, KeyLab integration for mapping/automation

#### 🧑‍🏫 Optional: Online Lessons
- **Platforms:** Preply, Superprof, TakeLessons, or Lessonface
- **Focus:** MIDI keyboard technique, improvisation, DAW help
- **Frequency:** 1 lesson/month

#### 📖 Additional Courses
- **Platforms:** Skillshare or Udemy
- **Search for:** "Piano for music producers", "Learn chords for electronic music"
- **Format:** One-time purchase or subscription for structured lessons

---

## Phase 2: Functional Playing + Electronic Jams (6–14 months)

### Equipment Budget

**Month 1:** $99 upgrade to Ableton Live Standard (full FX processor)  
**Month 2:** Volca Sampler 2 - 137 EUR  
**Month 3:** Speakers: ADAM AUDIO D3V BK - 298 EUR

### Focus
Play chords fluently in all keys, use both hands (bass + harmony), integrate with gear.

### Activities
- Learn basic improvisation (using pentatonic, minor/major scales)
- Set up simple DAW sessions (Ableton, FL Studio, etc.)
- Record chord loops, layer melodies over them
- Use Volcas for rhythm and texture while composing harmonies

### Expected Result
You'll be able to jam fluidly, compose full tracks, and start performing sets.

### Technical Skills

#### 🧱 Build Your Own FX Racks in Live
- Setup: Delay → Reverb → Chorus (mimic NTS-1)
- Map knobs to KeyLab for real-time control

### Recommended Courses

#### 🎓 Advanced Training
- **Slynk's Ableton FX & Sound Design course** (Skillshare)
- **Pianote:** Chords & Improv path (skip classical focus)

#### Skill Development Focus
- Learn how to layer samples, use MIDI FX, and EQ/filter
- Compose short clips using Volca Keys + Sampler in Ableton
- Practice full chord voicing with both hands (MIDI)

---

## Phase 3: Advanced Control and Emotional Expression (14+ months)

### Equipment Budget

**Month 1:** Volca Beats - 125 EUR  
**Month 2:** Volca Bass - 130 EUR  
**Month 3:** x4 input audio interface UMC404HD - 130 EUR

### Focus
Deep musical intuition, modulations, more complex harmony, expressive performance.

### Activities
- Compose emotionally driven pieces
- Learn modal progressions, seventh chords, voice leading
- Explore ambient, techno, IDM, lo-fi, or whatever niche you're drawn to

### Gear Setup
- **Hardware:** Volca Beats + Bass + audio interface
- **Software:** Advanced Ableton Live routing

### Advanced Training Resources

#### 🎚️ Mixing + Recording Courses
- **Free Option:** [Mixing for Producers (YouTube)](https://youtube.com) – free, practical
- **Paid Option:** Produce Like A Pro: Home Mixing (Udemy ~$20–30)

#### 🎹 Continued Learning
- **Melodics:** Scale/chord packs with beat-sync learning
- **Ableton Live:** External Instrument, audio channels, MIDI sync with Volcas

### Goals
- Record multi-channel sessions (via UMC404HD)
- Mix tracks in Ableton
- Jam with FX in real-time using MIDI controller + Live


